...
namespace FudgeMessage.Encodings
{
    ...
    public class FudgeXmlStreamReader : FudgeStreamReaderBase
    {
        ...
        public override short? TaxonomyId { get; }

        public override IFudgeTaxonomy Taxonomy
        {
            get { return FudgeContext.TaxonomyResolver.ResolveTaxonomy(TaxonomyId); }
        }
        ...
        {
            ReadEnvelope( FudgeElementNames.DEFAULT_ENVELOPE_ELEMENT);
        }


        public void ReadEnvelope(String envelopElementName)
        {
            CurrentElement = FudgeStreamElement.NoElement;
            if (HasNext)
            {
                // This first element could be an envelope
                if((reader.NodeType == XmlNodeType.Element) && (reader.Name == envelopElementName))
                {
                    var result = int.TryParse(reader.GetAttribute(FudgeElementNames.DEFAULT_ENVELOPE_ATTRIBUTE_PROCESSINGDIRECTIVES), out processingDirectives);
                    result = int.TryParse(reader.GetAttribute(FudgeElementNames.DEFAULT_ENVELOPE_ATTRIBUTE_SCHEMAVERSION), out schemaVersion);
                }
            }
        }
