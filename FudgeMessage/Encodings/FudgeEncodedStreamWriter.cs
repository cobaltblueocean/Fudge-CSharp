...
namespace FudgeMessage.Encodings
{
    ...
    public class FudgeEncodedStreamWriter : IFudgeStreamWriter
    {
        ...
        private static int WriteMsgEnvelopeHeader(BinaryWriter bw, short? taxonomyId, int messageSize, int version)// throws IOException
        {
            ...
            nWritten += 1;
            bw.Write((short)taxonomyId);      // REVIEW 2009-10-04 t0rx -- Should this be ushort?
            nWritten += 2;
        ...
        {
            WriteMsgEnvelopeHeader(outputWriter, TaxonomyId, messageSize, schemaVersion);
        }
